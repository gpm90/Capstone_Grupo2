...
function borrarPlanilla($archivo){
	...
	$rss = querys($queryDel,$tipobd_totvs,$conexion_totvs);
...
function insert_data($filial, $zitem, $oc, $cliente, $canal,
	...
	$mfield['ZC6_FILIAL']['value']=$filial;
	...
	$mfield['ZC6_INTCOD']['value']=$intcod;
	$mfield['ZC6_CODBAR']['value']=$codbar;
	$mfield['ZC6_PRCVEN']['value']=$neto;
	$mfield['ZC6_CANT']['value']=$cantidad;
	$mfield['ZC6_VALOR']['value']=$valor;
	$mfield['ZC6_LOCPAD']['value']=$locpad;
	$mfield['ZC6_UM']['value']=$um;
	$mfield['ZC6_SEGUM']['value']=$segum;
	$mfield['ZC6_INTDES']['value']=$intdes;
	$mfield['ZC6_FACTOR']['value']=$factor;
	...
	$mfield['ZC6_OKDIGI']['value']=$okdigit;	
	$mfield['ZC6_CONTA']['value']=$conta;
	$mfield['ZC6_ITEMCT']['value']=$itemcta;
	$mfield['ZC6_CLVL']['value']=$clvl;
	$mfield['ZC6_GRUPO']['value']=$grupo;
	$mfield['ZC6_CC']['value']=$cc;
...
function leer_archivo($archivo){
	...
	for ($i = 2; $i <= $numRows; $i++) {
		...
		$vCliente = '833827006'; 
		$vCanal = '4003';
		$vLocal = trim($cellLocal);
		$vDlocal = trim($cellDlocal);
		$vItem = '01'; 
		$vIntcod = trim($cellSKUmch);
		...
		$vCantidad = trim($cellUnidades);
		$vValor = $neto * $vCantidad;
		...
		$vClicod = trim($cellSKUcli);
		$vClibarcod = trim($upc);
		$vClides = trim($cellDescli);
		
		$vFecoc = date('Ymd', strtotime($cellFecemision. ' + 1 days'));
		$vFecemision = $hoy; 
		$vFecentrega = date('Ymd', strtotime($vFecoc. ' + 7 days'));
		$vArchivo = $archivo; 
		$vAdicional2 = $vDcto2; 
		$vAdicional3 = trim($cellCodprov); 
		$prcLista = lista_precios($vIntcod);
		$vRecno = recno();
		$vOKdigit = 'N'; 
		...
		insert_data($vFilial, $vZitem, $vOC, $vCliente, $vCanal,
					$vLocal, $vDlocal, $vItem, $vIntcod, $vCodbar,
					$vPrcven, $vCantidad, $vValor, $vLocpad, $vUM,
					$vSegum, $vIntdes, $vFactor, $vClicod, $vClibarcod,
					$vClides, $vFecoc, $vFecemision, $vFecentrega,
					$vArchivo,  $vAdicional2, $vAdicional3, 
					$vRecno,  $vOKdigit,  $vConta, $vItemcta, $vClvl,
					$vGrupo, $vCC, $vUsuario, $vDcto1, $vDcto2, $vPrcoc,$neto, $precio_cliente,
					$cod_dpto, $nombre_depto
		);

		if($cellSKUmch == '999999999999999'){
			echo "ERROR: <strong> $cellCodprov, $cellSKUcli, $cellDescli </strong> <br>";
		}
...
function reprocesar_planilla($archivo){
	...
	$rz6 = querys($queryrep, $tipobd_totvs, $conexion_totvs);
