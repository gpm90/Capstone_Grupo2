...
function valida_formato($archivo){
	...
	$expectedColumns = [
		'N° Orden',	'Línea',	'Cód. Departamento',	'Departamento',	'Estado',	'Tipo de Orden',	
		'Cód. Local Entrega',	'Local Entrega',	'Cód. Local Destino',	'Local Destino',	'Fecha Vigencia',	
		'Fecha Vto.',	'Emitida',	'SKU Paris',	'Cód. Prod. Prov.',	'Cód. Barra Paris',	'Tipo Flujo',	
		'Descripción',	'Color',	'Dimensión',	'Talla',	'Innerpack',	'Precio Normal',	'Precio Oferta',	
		'Precio Costo',	'Descuento',	'Costo Neto',	'Solicitado',	'Recibido',	'En Despacho',	'Pendiente',	
		'Tolerancia Mín.',	'Tolerancia Máx.',	'eanproveed1',	'eanproveed2',	'eanproveed3',	'DESC-1',	'DESC-2'
	];
	
	// Ruta del archivo Excel
	$filePath = '../../archivos_subidos/archivos_distribucion/paris_distribucion/' . $archivo;
...
function leer_archivo($archivo){
	...
	if($codigo_monarch == '999999999999999'){
		echo "ERROR-01: SKU <strong> $sku, $cod_barra_paris, $descript_paris </strong> SIN EQUIVALENCIA, AGREGAR EQUIVALENCIA Y VOLVER A SUBIR ARCHIVO<br>";
	...
	// $queryup = "update ".TBL_ZEQ." SET ZEQ_DEPTO='$cod_depto', ZEQ_DDEPTO='$departamento', ZEQ_ADD1='$talla' 
	...
	if($rss){	 
		...
		while($fila = ver_result($rss, $tipobd_totvsDev2)){
			...
			$bkpprun,	'$rateio',	'$codbar',		$unempq,	'$capac',	$recno, $precio_lista,$b1_factor)";
			 ...
			 //echo "SC6 : ".$insert.'<br>';
			$rs = querys($insert,$tipobd_totvsDev2,$conexion_totvsDev2);
			$queryup_rec = "UPDATE ".TBL_ZC6010."  SET ZC6_SC6REC=$recno where zc6_intcod='$produto' and zc6_num='$num'";
