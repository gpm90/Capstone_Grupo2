...
function ver_informe($canal, $codigo_monarch, $fec_ini, $fec_fin){
	...
	$fec_ini = str_replace(array('/'),'',$fec_ini);
		$parte1 = substr($fec_ini,4,8); //12
		$parte2 = substr($fec_ini,2,2); //345
		$parte3 = substr($fec_ini,0,2); //456
		$fec_ini = $parte1.$parte2.$parte3;
	$fec_fin = str_replace(array('/'),'',$fec_fin);
		$parte1 = substr($fec_fin,4,8); //12
		$parte2 = substr($fec_fin,2,2); //345
		$parte3 = substr($fec_fin,0,2); //456
		$fec_fin = $parte1.$parte2.$parte3;
...
if(isset($_GET["ver_pedido"])){
	$canal 	= $_GET["canal"];
	$codigo_monarch 	= $_GET["cod_monarch"];
	$fec_ini = $_GET["fec_ini"];
	$fec_fin = $_GET["fec_fin"];

	ver_informe($canal, $codigo_monarch, $fec_ini, $fec_fin);
}